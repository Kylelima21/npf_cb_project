...
ui <- function() {
  ...
  pagePiling(
    ...
    pageSectionImage(
      ...
      h1(textOutput("title"), class = "header-home shadow-dark"),
      
      h3("Citizen Science Explorer", class = "subheader-home shadow-dark"),
    ...
    pageSectionImage(
      ...
      div(class='container',
          div(class='row',
              div(class='col-lg-6',
                  div(class="obs-box-inat", img(src="img/inat.png", class="obs-logos"),
                      div(class="sep-line"),
                      div(class="obs-text", 
                          tags$ul(
                            tags$li(class="li-text", textOutput("total_observers")),
                            tags$li(class="li-text", textOutput("top_taxa")),
                            tags$li(class="li-text", textOutput("top_sp")))))),
              div(class='col-lg-6',
                  div(class="obs-box-ebird", img(src="img/ebird.png", class="obs-logos"),
                      div(class="sep-line"),
                      div(class="obs-text", 
                          tags$ul(
                            tags$li(class="li-text", textOutput("total_observers_e")),
                            tags$li(class="li-text", textOutput("top_taxa_e")),
                            tags$li(class="li-text", textOutput("top_sp_e")))))),
              div(class='col-lg-12',
                  div(class="percent-plot",
                      h3(class="percent-title", textOutput("percent_text")),
                      plotOutput("percentplot", height = "11rem", width = "98%")))
          )),
    ...
    pageSection(
      ...
      absolutePanel(class = "picker-box",
...
server <- function(input, output, session) {
  ...
  output$percentplot <- renderPlot({
    the_data %>%
      ...
      geom_text(aes(x = category, y = percent, label = percent, group = source),
                position = position_stack(vjust = 0.5), size = 9, color = "#eae7e7") +
  ...
  output$percent_text <- renderText({
    ...
    paste0(dat$percent[1], "% of the data from the last 7 days was collected by eBird, 
           while ", dat$percent[2], "% was collected by iNaturalist")
  })
  ...
  output$top_sp_e <- renderText({
    ...
    paste("The most commonly reported species was", species$common.name[1],
          "appearing on", species$count[1], "checklists.", sep = " ")
  })
  
  ## Text output for the group with the most observations
  output$top_taxa_e <- renderText({
    taxon <- the_data %>%
      filter(source == "eBird") %>% 
    ...
    paste("Across these checklists", length(taxon$common.name),
          "different species were recorded.", sep = " ")
  })
  
  ## Text output for the number of total obs
  output$total_observers_e <- renderText({
    observers <- the_data %>%
      filter(source == "eBird") %>% 
      group_by(checklist) %>%
    ...
    paste(length(observers$checklist), "checklists were submitted in the park in the last 7 days.", sep = " ")
  })
